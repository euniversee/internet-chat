...
function printStartupBanner() {
  ...
  console.log(`${color.white}internet-chat${color.reset}\n`);
  console.log(`${color.orange}${logo.join("\n")}${color.reset}`);
  console.log(`\n${color.orange}Internet Chat v1.0${color.reset}`);
  console.log(`${color.dim}Real-time group chat fully deployed to the internet${color.reset}\n`);
  console.log(line);
  console.log(`${color.green}?${color.reset} Status        : ${color.green}RUNNING (${isProd ? "PRODUCTION" : "DEVELOPMENT"})${color.reset}`);
  console.log(`${color.green}>${color.reset} Port binding  : ${color.white}${PORT}${color.reset}`);
  console.log(`${color.green}>${color.reset} Upload limit  : ${color.white}${formatBytes(MAX_UPLOAD_BYTES)} per file${color.reset}`);
  
  if (isProd) {
    console.log(`${color.green}>${color.reset} mDNS          : ${color.dim}disabled (production mode)${color.reset}`);
    console.log(line);
    return;
  }
